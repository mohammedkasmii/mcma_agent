...
| **G1** Pure deterministic fail-closed core | INC-05 | Phase 2 | import-linter green; `plan_hash` determinism; every unknown/ambiguous domain case fails closed; **repair workflow is typed (`RepairWorkflow`) and included in plan hashing; both deterministic builders and their registry entries are covered by tests** |
| **G2** Write-safety core (no live writes) | INC-09 | Phase 3 | dry-run has no writer; final endpoints abort; unknown requests fail closed; identity + exact-IdRubrique fail closed; charge-mutuelle never directly written; **exact Mode Normal (Ajouter/`createRapportDefDet`) and PEC (pencil/`updateDevisDet`) mock workflow tests pass; the observed portal workflow agrees with the executable plan (mismatch fails closed pre-mutation); native financial verification is mandatory; a failed/stale/missing calculation or summary mismatch prevents readiness (`WRITE_ABORTED`); final dossier actions are blocked** — all against the mock; live writes still disabled |
...
| **G5** Live-write gate | INC-23 | any live row write | confirmed row-op contracts from approved evidence — **the exact Mode Normal native trigger + summary-verification contract confirmed (currently UNCONFIRMED, `PORTAL_ROW_WORKFLOWS.md` §3.1) and the exact PEC row (`updateDevisDet`) + native (`DevisCalculerMontantCharge`) contracts confirmed; `confirmed_row_ops` records are commit-bound (valid only for the exact deployed commit); no live writer is constructible before all evidence is approved**; full safety suite green; write-enable gate satisfied; final endpoints still blocked; **explicit owner approval** before the supervised canary |
