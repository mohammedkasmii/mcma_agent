...
`browser/mode_normal.py:122-144` sets `MontantChargeMutuelle = MontantReparation` and `MontantChargeSocietaire = '0'` after native calc, zeroing franchise/vétusté/part-responsabilité. Violates the resolved rule (`BUSINESS_RULES.md` B.3 — charge-mutuelle authority). Baseline classification: **Mode Normal baseline is unsafe — a prohibited direct charge-split overwrite.** **PEC baseline is partially aligned**: `mode_conventionne` delegates calculation to SinAuto and does not directly write the split, but it does not prove native-trigger completion or perform exact financial-summary verification, so it is **not fully target-compliant**. Full target compliance in both workflows requires native trigger + summary read-back + exact verification before `READY_FOR_HUMAN_REVIEW` (refer to `docs/architecture/PORTAL_ROW_WORKFLOWS.md`).
